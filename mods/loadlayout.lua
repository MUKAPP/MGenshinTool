...
local function setattribute(root,view,params,k,v,ids)
  if k=="layout_x" then
   ...
   elseif k=="addTextChangedListener" and type(v)=="table" then --设置属性
    view.addTextChangedListener(v)
   elseif k=="thumbColor" then
    view.ThumbDrawable.setColorFilter(PorterDuffColorFilter(checkNumber(v),PorterDuff.Mode.SRC_ATOP))
   elseif k=="trackColor" then
    view.TrackDrawable.setColorFilter(PorterDuffColorFilter(checkNumber(v),PorterDuff.Mode.SRC_ATOP))
   elseif k=="onTouch" then --设置onTouch事件接口
    view.onTouch=v
   elseif k=="onDraw" then
    view.getViewTreeObserver().addOnDrawListener(ViewTreeObserver.OnDrawListener{
      onDraw=function(...)v(view,...)end,
    })
   elseif k=="onGlobalLayout" then
    view.getViewTreeObserver().addOnGlobalLayoutListener(ViewTreeObserver.OnGlobalLayoutListener{
      onGlobalLayout=function(...)v(view,...)end,
    })
   elseif k=="onPageChangeListener" then
    view.setOnPageChangeListener(PageView.OnPageChangeListener{
      onPageScrolled=function(...)
        v["onPageScrolled"](view,...)
      end,
      onPageSelected=function(...)
        v["onPageSelected"](view,...)
      end
    })
