...
function onCreate()
    ...
    adp.add { __type = 4, subtitle = "实时便笺通知栏常驻", message = "开启此项即开启 实时便笺 功能的通知栏常驻，同时也会开启树脂提醒功能。",
        status = { Checked = Boolean.valueOf(实时便笺通知栏常驻) } }
    adp.add { __type = 4, subtitle = "实时便笺通知信息简化", message = "如果觉得实时便笺通知信息过于繁杂的话或许可以简化一下。",
        status = { Checked = Boolean.valueOf(实时便笺通知信息简化) } }
    adp.add { __type = 4, subtitle = "进入软件时自动签到", message = "“实时便笺通知栏常驻”启用时该项才会生效。进入软件时自动签到。",
        status = { Checked = Boolean.valueOf(进入软件时自动签到) } }
    adp.add { __type = 4, subtitle = "自动签到", message = "“实时便笺通知栏常驻”启用时该项才会生效。自动签到时间：8:00~22:00，每到整点自动签到。",
        status = { Checked = Boolean.valueOf(自动签到) } }
    adp.add { __type = 2, subtitle = "实时便笺刷新时间", message = "每隔多少分钟刷新一次实时便笺" }
    adp.add { __type = 2, subtitle = "关闭对该应用的电池优化",
        message = "“实时便笺”及“自动签到”需要应用在后台运行，点击进入电池优化界面，选择无限制，并且在多任务界面锁定本软件。" }
    adp.add { __type = 2, subtitle = "悬浮浏览器主页", message = "设置悬浮浏览器的默认加载页面" }
    adp.add { __type = 1, title = "界面" }
    adp.add { __type = 4, subtitle = "跟随系统夜间模式", message = "若开启此项不跟随系统夜间模式，则您的系统不支持原生夜间模式API",
        status = { Checked = Boolean.valueOf(跟随系统夜间模式) } }
    adp.add { __type = 4, subtitle = "夜间模式", message = "“跟随系统夜间模式”启用时该项不生效",
        status = { Checked = Boolean.valueOf(夜间模式) } }
    adp.add { __type = 4, subtitle = "更深的夜间模式背景", message = "为夜间模式更换更深的背景色",
        status = { Checked = Boolean.valueOf(更深的夜间模式背景) } }
    adp.add { __type = 2, subtitle = "主题", message = "夜间模式开启时主题不生效" }
    adp.add { __type = 4, subtitle = "主页垫高导航栏",
        message = "部分系统开启全面屏手势之后仍然会给软件提供导航栏高度，主页就会出现导航栏位置空一片的情况，关闭此项以解决该问题",
        status = { Checked = Boolean.valueOf(垫高导航栏) } }
    adp.add { __type = 2, subtitle = "校准屏幕大小",
        message = "如果你的悬浮浏览器等功能的尺寸不正常，请退出小窗等状态、正常打开该页面并将屏幕竖置，点击此项来校准屏幕大小" }
    ...
    adp.add { __type = 4, subtitle = "开启开发者模式", message = "便于调试代码（可能会拖慢性能）",
        status = { Checked = Boolean.valueOf(开发者模式) } }
