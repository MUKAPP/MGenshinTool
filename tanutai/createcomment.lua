...
function onCreate()
  ...
  _send.onClick=function()
    ...
    加载对话框("正在发布","请稍等…",0)
    tanutai.createComment(article_id,edit.Text,
    function(content)
      关闭对话框()
      if content.code==0 then
        edit.Text=""
  ...
  if mukactivity.getData("tanutai_comment_draft") then
    comment_draft=JSON.decode(mukactivity.getData("tanutai_comment_draft"))
    if comment_draft[tostring(article_id)]
      edit.Text=tostring(comment_draft[tostring(article_id)])
    end
   else
    comment_draft={}
    mukactivity.setData("tanutai_comment_draft",JSON.encode(comment_draft))
  end
end

function onStop()
  xpcall(function()
    comment_draft[tostring(article_id)]=tostring(edit.Text)
    mukactivity.setData("tanutai_comment_draft",JSON.encode(comment_draft))
    end,function(e)
    print(e)
  end)
