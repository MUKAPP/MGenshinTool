...
xpcall(function()
    ...
    function dailyNote2(retry)
        ...
        if cookie == nil then
            sendNotification("实时便笺", "你还没有登录，点击登录米游社账号并设置默认账号", "1"
                , "实时便笺",
        ...
        Http.get("https://api-takumi.mihoyo.com/binding/api/getUserGameRolesByCookie?game_biz=hk4e_cn",
            ...
            function(code, content)
                ...
                Http.get("https://api-takumi-record.mihoyo.com/game_record/app/genshin/api/dailyNote?server=" ..
                    ...
                    function(code, content)
                        ...
                        if content.retcode == 0 then
                            ...
                            if this.getSharedData("Note_Notification_Simple") == "false" then
                                ...
                                add("周本减半 " .. data.remain_resin_discount_num ..
                                ...
                                if data.resin_recovery_time == "0" then
                                    ...
                                    add("树脂 " ..
                                        ...
                                        "/" ..
                                        data.max_resin ..
                                        " - " .. secondtostring(tointeger(data.resin_recovery_time)) .. " 回满")
                                end
                            ...
                            if tointeger(data.current_resin) == 40 or tointeger(data.current_resin) == 80 or
                                tointeger(data.current_resin) == 120 then
                                if lastresin ~= tointeger(data.current_resin) then
                                    Log.d("NoteService",
                                        tostring("树脂提醒 " .. lastresin .. "->" .. tointeger(data.current_resin)))
                                    sendNotification("树脂提醒", "你的原粹树脂已恢复到 " ..
                                        data.current_resin, "2", "树脂提醒",
                            ...
                            if tointeger(data.current_resin) == 159 then
                                if lastresin ~= tointeger(data.current_resin) then
                                    Log.d("NoteService",
                                        tostring("树脂提醒 " .. lastresin .. "->" .. tointeger(data.current_resin)))
                                    sendNotification("树脂提醒", "你的原粹树脂马上就要满了，已恢复到 "
                                        .. data.current_resin, "2", "树脂提醒",
                                        "main.lua", false, true, true)
                                end
                            end
                            ...
                            if tointeger(data.current_resin) == 160 then
                                if lastresin ~= tointeger(data.current_resin) then
                                    Log.d("NoteService",
                                        tostring("树脂满了 " .. lastresin .. "->" .. tointeger(data.current_resin)))
                                    sendNotification("树脂提醒", "你的原粹树脂满了！溢出了呜呜呜", "2"
                                        , "树脂提醒",
                                        "main.lua", false, true, true)
                                end
                            end
                            ...
                            if content.message:find("not public") or content.message:find("Data not") then
                                sendNotification("实时便笺",
                                    "获取失败：" .. content.message .. "\n请到米游社内找到 我的-我的角色-实时便笺 开启实时便笺功能"
                                    , "1", "实时便笺",
                                    "main.lua", true, false, false)
                                return true
                            end
                            if content.retcode == 1034 then
                                sendNotification("实时便笺", "获取失败：请进入软件手动过验证", "1",
                                    "实时便笺",
                                    "main.lua", true, false, false)
                                return true
                            end

                            sendNotification("实时便笺", "获取失败：错误码 " ..
                                content.retcode .. "；错误信息 " .. content.message, "1", "实时便笺",
                                "main.lua", true, false, false)
                        end
                    end)
            ...
            local function next()
                ...
                nextTicker.onTick = function()
                    ...
                    if signed + 1 <= #all_uid then
                        ...
                        for i, v in ipairs(sign_results) do
                            if v[2]:find("签到过了") or v[2]:find("成功") or v[2]:find("签到过啦") then
                                signok = signok + 1
                            end
                        end
                        ...
                        sendNotification("自动签到结果", "成功: " ..
                            signok .. " 失败: " .. (#all_uid - signok) .. "  点击查看详情", "3", "自动签到",
                ...
                xpcall(function()
                    ...
                    Http.get(
                        ...
                        if code ~= 200 then
                            ...
                            sign_results[#sign_results + 1] = { "账号 " .. nam .. " 签到失败", "请求失败，错误码："
                                .. code, cookie }
                            next()
                            haserror = true
                            return true
                        ...
                        if is_sign == true then
                            sign_results[#sign_results + 1] = { nickname .. "  UID: " .. uid .. " (" .. region_name ..
                                ")", [[今日奖励: ]] ..
...
签到结果: ]]                                                             .. message, cookie }
                    ...
                    sign_results[#sign_results + 1] = { "账号 " .. nam .. " 签到失败", "请求失败，未知错误："
                        .. dump(content), cookie }
                    next()
                    return true
                end)
        ...
        function getUids()
            ...
            Http.get("https://api-takumi.mihoyo.com/binding/api/getUserGameRolesByCookie?game_biz=hk4e_cn",
                cookie, nil, nil, function(code, content)
                if code ~= 200 then
                    sign_results[#sign_results + 1] = { "账号 " .. nam .. " 签到失败", "获取账号信息失败，错误码："
                        .. code, cookie }
                    next()
                    haserror = true
                    return true
        ...
        Http.get("https://bbs-api.mihoyo.com/apihub/api/home/new?cpu=mt6785v/cc&device=Xiaomi%20Redmi%20Note%208%20Pro&gids=2"
            ...
            function(code, content)
                if code ~= 200 then
                    haserror = true
                    for i, v in ipairs(sign_results) do
                        if v:find("签到过了") or n:find("成功") or n:find("签到过啦") then
                            signok = signok + 1
                        end
                    end
...
function openService()
    ...
    autoSign.onTick = function()
        if tointeger(os.date("%H")) >= 8 and tointeger(os.date("%H")) <= 22 then
            if tointeger(os.date("%M")) == 00 then
                if this.getSharedData("Note_Auto_Sign") == "true" then
                    mys_signIn()
                end
            end
        end
    ...
    autoSign.start()
    if this.getSharedData("Note_Auto_Sign2") == "true" then
        mys_signIn()
    end
end
