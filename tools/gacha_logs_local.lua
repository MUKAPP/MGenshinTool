...
function onCreate()
        ...
        onClick = function() end;
        {
            ...
            id = "tlb";
            {
                ...
                onClick = function(v)
                    ...
                    写入文件(内置存储文件("gacha_export.tmp"), 读取文件(新内置存储文件("GachaLogs/"
                        .. v.Text .. ".json")))
                        ...
                        layout_height = "-1",
                        {
                            ...
                            },
                            {
                                ...
                                layout_marginBottom = "8dp",
                                {
                                    ...
                                    onClick = function()
                                        关闭对话框()
                                        local fileName = "抽卡记录导出_" ..
                                            os.date("%Y-%m-%d_%H-%M-%S") .. "_UIGF.json"
                                        创建文件(新内置存储文件(fileName))
                                        写入文件(新内置存储文件(fileName),
                                            读取文件(新内置存储文件("GachaLogs/" .. v.Text .. ".json")))
    ...
    默认提示 = "点击查看本地记录，通过抽卡链接分析记录和导入JSON分析记录都会自动合并至本地记录。长按可以导出JSON。"
    ...
    adp = LuaRecyclerViewAdapter(LuaAdapterCreator({
        ...
        onBindViewHolder = function(holder, position)
            ...
            if data[position + 1][1] ~= 默认提示 then
