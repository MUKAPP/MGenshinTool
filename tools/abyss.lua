...
function onCreate()
  ...
  adp1=LuaRecyclerViewAdapter(LuaAdapterCreator({
    ...
    getItemViewType=function(position)
      if position==0
        return 1
       elseif position==#utilizationRate+1
        return 3
       else
        return 2
      end
    ...
    onBindViewHolder=function(holder,position)
      ...
      if viewType==1 then
        ...
        view.info.Text="使用率指持有该角色的玩家的使用频率。\n当期有效深渊数据用户数量："..spiralAbyssFullStar.."。\n欢迎上传您的深渊记录来与我们共建深渊数据库。"
       elseif viewType==3 then
       elseif viewType==2 then
        Glide.with(this)
        ...
        TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(view.name,4,12,2,TypedValue.COMPLEX_UNIT_SP);
        TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(view.rate,4,12,2,TypedValue.COMPLEX_UNIT_SP);
        if utilizationRate[position].rate>=80
          view.rate.textColor=0xff00BFA5
         elseif utilizationRate[position].rate>=40
          view.rate.textColor=0xff2979FF
         elseif utilizationRate[position].rate>=20
  ...
  adp2=LuaRecyclerViewAdapter(LuaAdapterCreator({
    ...
    getItemViewType=function(position)
      if position==0
        return 1
       elseif position==#attendanceRate+1
        return 3
       else
        return 2
      end
    ...
    onBindViewHolder=function(holder,position)
      ...
      if viewType==1 then
       ...
       elseif viewType==2 then
        ...
        TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(view.name,4,12,2,TypedValue.COMPLEX_UNIT_SP);
        TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(view.rate,4,12,2,TypedValue.COMPLEX_UNIT_SP);
        if attendanceRate[position].rate>=80
          view.rate.textColor=0xff00BFA5
         elseif attendanceRate[position].rate>=40
          view.rate.textColor=0xff2979FF
         elseif attendanceRate[position].rate>=20
          view.rate.textColor=0xffFFAB00
         else
          view.rate.textColor=0xffFF1744
        end
    ...
    onBindViewHolder=function(holder,position)
      ...
      if viewType==1 then
       ...
       elseif viewType==2 then
        ...
        TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(view.name,4,14,2,TypedValue.COMPLEX_UNIT_SP);
        TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(view.rate,4,12,2,TypedValue.COMPLEX_UNIT_SP);
        if holdingRate[position].rate>=80
          view.rate.textColor=0xff00BFA5
         elseif holdingRate[position].rate>=40
          view.rate.textColor=0xff2979FF
         elseif holdingRate[position].rate>=20
          view.rate.textColor=0xffFFAB00
         else
          view.rate.textColor=0xffFF1744
        end
        ...
        TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(view.constellations1,4,12,2,TypedValue.COMPLEX_UNIT_SP);
        TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(view.constellations2,4,12,2,TypedValue.COMPLEX_UNIT_SP);
      end
    end,
  }))
  ...
  avatarData=JSON.decode(读取文件(activity.getLuaDir("res/metadata/Avatar.json")))
...
加载对话框内容("已完成："..ok.."/3")
  ...
  hutaoapi.statistics.overview(function(data)
    ...
    if spiralAbyssFullStar<1000
      spiralAbyssFullStar=spiralAbyssFullStar.."（当前数据过少可能会导致结果不准确）"
    end

    hutaoapi.statistics.avatar_utilizationRate(function(data)
      --使用率
      for i,v in ipairs(data.data)
        if v.floor==12
          for i2,v2 in ipairs(v.ranks)
            for i3,v3 in ipairs(avatarData)
              if v2.item==v3.Id
                utilizationRate[#utilizationRate+1]=
                {
                  "name"=v3.Name,
      ...
      table.sort(utilizationRate,function(a,b)
    ...
    hutaoapi.statistics.avatar_attendanceRate(function(data)
      ...
      for i,v in ipairs(data.data)
        if v.floor==12
          for i2,v2 in ipairs(v.ranks)
            for i3,v3 in ipairs(avatarData)
              if v2.item==v3.Id
                attendanceRate[#attendanceRate+1]=
                {
                  "name"=v3.Name,
                  "rate"=tointeger(v2.rate*10000)/100,
                  "icon"=v3.Icon,
      ...
      table.sort(attendanceRate,function(a,b)
        return a.rate>b.rate
      end)
    ...
    hutaoapi.statistics.avatar_holdingRate(function(data)
        ...
        for i3,v3 in ipairs(avatarData)
          if v.avatarId==v3.Id
            ...
            for i=1,#v.constellations do
              if tointeger(i/2)~=i/2
                if constellations1==""
                  constellations1=v.constellations[i].item.."命 "..(tointeger(v.constellations[i].rate*10000)/100).."%"
                 else
                  constellations1=constellations1.."\n"..v.constellations[i].item.."命 "..(tointeger(v.constellations[i].rate*10000)/100).."%"
                end
               else
                if constellations2==""
                  constellations2=v.constellations[i].item.."命 "..(tointeger(v.constellations[i].rate*10000)/100).."%"
                 else
                  constellations2=constellations2.."\n"..v.constellations[i].item.."命 "..(tointeger(v.constellations[i].rate*10000)/100).."%"
                end
              end
            end
            holdingRate[#holdingRate+1]=
            {
              "name"=v3.Name,
      ...
      table.sort(holdingRate,function(a,b)
        return a.rate<b.rate
      end)
