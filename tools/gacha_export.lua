...
function querySplit(query)
    ...
    for i, v in ipairs(queryTableBefore) do
...
function mergeSplit(queryTable)
    local query = ""
    for i, v in pairs(queryTable) do
        if query ~= "" then
            query = query .. "&" .. i .. "=" .. v
        else
            query = i .. "=" .. v
...
function getGachaLogs(url)
    ...
    function nextURL(url)
        ...
        if nt ~= true then
            ...
            info.Text = "正在获取 " .. gacha_table_old[last_type].name .. " 第" ..
                tostring(tointeger(lastpage + 1)) .. "页"
        else
            ...
            query_table["end_id"] = "0"
            nt = false
            if nextType(last_type) == false then
                printLog("nextURL", "完成")
                info.Text = "获取完成"
    ...
    function addLogs(tab, url)
        ...
        if tab.retcode ~= 0 then
            if tab.message == "authkey timeout" then
        ...
        if #tab.data.list > 0 then
            for i, v in ipairs(tab.data.list) do
                if v.gacha_type ~= last_type then
                    last_type = v.gacha_type
                    if v.gacha_type == "400" then
                        last_type = "301"
                    end
                end
...
local fs = files.open(新内置存储文件(), 'f')
    ...
    :on(function(file)
        xpcall(function()
            ...
            if content2.info.uigf_version then
...
if not 文件是否存在(新内置存储文件("GachaLogs")) then
